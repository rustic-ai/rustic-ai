...
@pytest.mark.asyncio
async def test_memory_backend_upsert_and_search():
    schema = _schema_text()
    be = InMemoryKBIndexBackend()
    ...
    # Ensure scalar mapping from meta and chunk worked
...
@pytest.mark.asyncio
async def test_memory_backend_unknown_vector_is_skipped():
    schema = _schema_text()
    be = InMemoryKBIndexBackend()
    await be.ensure_ready(schema=schema)

    # EmittedRow contains only an unknown vector space; should not error, but search will not find it
