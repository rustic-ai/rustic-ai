...
class ImageFixedByteChunker(ChunkerPlugin):
    """Split image payload into fixed-size byte segments (dependency-free).

    Emits ImageChunk records and writes segment bytes via KnolUtils so downstream
    consumers can retrieve content-addressed parts.
    """
    ...
    async def split(self, knol: Knol, *, fs: FileSystem, library_path: str = LIBRARY_DIR) -> AsyncGenerator[ChunkBase, None]:  # type: ignore[override]
        ...
        async with KnolUtils.stream_content(knol, fs, library_path, mode="rb", chunk_size=self.chunk_bytes) as stream:
            async for block in stream:
                # Persist derived bytes for each segment; extension defaults for images
                chunk = ImageChunk(
