...
class TestRayRedisIntegration(IntegrationTestABC):
    ...
    def wait_time(self) -> float:
        # Ray agents need more time to initialize in distributed mode
        return 0.5
    ...
    def test_agents_communication(
        ...
        """Override base test with Ray + Redis specific timing and retry logic."""
        ...
        # Try multiple times to get messages (retry mechanism for distributed execution)
        max_attempts = 3
        for attempt in range(max_attempts):
            ...
            print(f"Ray Redis test attempt {attempt + 1}: Publishing initial message...")
            local_test_agent.publish_initial_message()

            # Allow longer time for message processing in Ray + Redis distributed setup
            ...
            print(
                f"Ray Redis test attempt {attempt + 1}: Only got {len(local_test_agent.captured_messages)} messages, retrying..."
            )
        ...
        print(f"Ray Redis test captured {len(local_test_agent.captured_messages)} messages")
        ...
        ), f"Ray Redis test: Expected at least 2 messages after {max_attempts} attempts, got {len(local_test_agent.captured_messages)}"
