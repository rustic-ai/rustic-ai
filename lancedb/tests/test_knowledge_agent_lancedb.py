...
from rustic_ai.lancedb.kbindex_backend_lancedb import LanceDBKBIndexBackendResolver
...
def dependency_map(tmp_path):
    base = Path(tmp_path) / "kb_agent_tests_lancedb"
    return {
        ...
        "kb_backend": DependencySpec(
            class_name=LanceDBKBIndexBackendResolver.get_qualified_class_name(),
            properties={"uri": str(tmp_path / ".lancedb")},
        ),
    }
...
class TestKnowledgeAgentLanceDB:
    ...
    def test_index_and_search(self, agent_spec, dependency_map, generator):
        agent, results = wrap_agent_for_testing(agent_spec, dependency_map=dependency_map)
    ...
    def test_hybrid_rrf_explain(self, agent_spec, dependency_map, generator):
        agent, results = wrap_agent_for_testing(agent_spec, dependency_map=dependency_map)
        ...
        agent._on_message(
            _build_message(
                generator,
                KBSearchRequest(text="three", limit=3, hybrid=HybridOptions(), explain=True).model_dump(),
                get_qualified_class_name(KBSearchRequest),
            )
        )
